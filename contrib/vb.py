...
from contrib.penalty_functions import penalty_c
...
def init_sigma(w0, DGx, DGy, Gx2, GxGy, Gy2, Lamb, Lama_horiz, Lama_vert, params):
    M, N = DGx.shape[0:2]
...
def m_step(dw, w0, s, D2, DGx, DGy, Gx2, GxGy, Gy2, params):
    du = dw[:, :, 0]
    dv = dw[:, :, 1]
    su = s[:, :, 0]
    sv = s[:, :, 1]
    ...
    # Contribution of the posterior variance (forward)
    # horizontal
    sux = np.empty_like(su)
    sux[:, 0:-1] = su[:, 1:] + su[:, 0:-1]
    sux[:, -1] = 2*su[:, -1]
    suy = np.empty_like(su)
    suy[0:-1, :] = su[1:, :] + su[0:-1, :]
    suy[-1, :] = 2*su[-1, :]
    # vertical
    svx = np.empty_like(sv)
    svx[:, 0:-1] = sv[:, 1:] + sv[:, 0:-1]
    svx[:, -1] = 2*sv[:, -1]
    svy = np.empty_like(v)
    svy[0:-1, :] = sv[1:, :] + sv[0:-1, :]
    ...
    # horizontal
    tmp = ux*ux + vx*vx + sux + svx
    psi = params['psi']
    Lama_horiz = psi(tmp, params)

    # vertical
    tmp = uy*uy + vy*vy + suy + svy
    ...
    for scale in range(max_scale, -1, -1):
        ...
        for l in range(params['nwarp']):
            ...
            for k in range(params['niter']):
                ...
                Lamb, Lama_horiz, Lama_vert = m_step(dw, w, s, D2, DGx, DGy, Gx2, GxGy, Gy2, params)
                dw, s = e_step(dw, w, DGx, DGy, Gx2, GxGy, Gy2, Lamb, Lama_horiz, Lama_vert, params)
